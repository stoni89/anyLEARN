...
var user = {
    ...
    {
        return db.query('SELECT u.user_id, u.vorname, u.eintritt, u.nachname, u.name, u.kuerzel, u.rollen_id, u.kategorie_id, u.mail, u.istAktiv, r.rolle, k.kategorie FROM users u ' +
    ...
    getAllUserAktivAzubiAndMe: function(user_id, callback)
    {
    ...
    getAllUserAktivAzubi: function(callback)
    {
        return db.query('SELECT u.user_id, u.vorname, u.nachname, u.eintritt, u.name, u.kuerzel, u.rollen_id, u.kategorie_id, u.mail, u.istAktiv, r.rolle, k.kategorie FROM users u ' +
                        'INNER JOIN rollen r ON u.rollen_id = r.rollen_id ' +
                        'INNER JOIN kategorie k ON k.kategorie_id = u.kategorie_id ' +
                        'WHERE istAktiv = true AND u.rollen_id = 1 ' +
                        'ORDER BY u.name', callback);
    },
    ...
    {
        return db.query('SELECT u.user_id, u.mail, u.vorname, u.eintritt, u.name, u.rollen_id, u.istAktiv, r.rolle FROM users u INNER JOIN rollen r ON (r.rollen_id = u.rollen_id) WHERE mail = ' + '"' + mail + '"', callback);
    },
    ...
    {
      return db.query('UPDATE users SET vorname=?, eintritt=?, nachname=?, name=?, kategorie_id=?, rollen_id=?, kuerzel=?, mail=?, istAktiv=? ' +
                      'WHERE user_id=?', [postdata.vorname, postdata.eintritt, postdata.nachname, postdata.name, postdata.kategorie_id, postdata.rollen_id, postdata.kuerzel, postdata.mail, postdata.istAktiv, postdata.user_id], callback)
    }
}
